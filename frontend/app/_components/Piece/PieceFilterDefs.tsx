...
import { METAL_THEMES, TEXT_EMBOSS_FILTER_ID, pieceFilterId } from './metalThemes';
...
export default function PieceFilterDefs() {
  return (
    <svg width="0" height="0" style={{ position: 'absolute' }} aria-hidden="true">
      <defs>
        {(Object.keys(METAL_THEMES) as BodyColor[]).flatMap((bodyColor) =>
          Object.values(ARCHETYPES).map((archetype) => (
            <filter
              key={pieceFilterId(bodyColor, archetype)}
              id={pieceFilterId(bodyColor, archetype)}
              x="-20%"
              y="-20%"
              width="200%"
              height="200%"

...
11. **Missing or wrong Pydantic model suffixes** — Two-tier naming convention applies:
    - **Operation-specific models** (used by a single handler): `{Action}{Resource}Request` / `{Action}{Resource}Response`, where the action is a verb matching the operation (e.g. `CreateClauseRequest`, `SearchClauseResponse`, `BatchUpdateVectorResponse`).
    - **Shared resource representations** (returned unchanged by multiple handlers such as create, read, and update): `{Resource}Response` only, with no action prefix (e.g. `ClauseResponse`, `ExampleResponse`).
...
17. **Route function name does not match CRUD convention** — Route handler names in `crud.py` must follow the CRUD pattern (`create_*`, `batch_create_*`, `read_*`, `read_*_by_*`, `search_*_by_*`, `update_*_*`, `delete_*`, `batch_delete_*`). Non-crud route files (e.g. `match.py`, `process.py`, `grammar.py`) may use any descriptive verb-phrase name that reflects the operation. Flag any handler in a `crud.py` file whose name deviates; do not flag handlers in other route files.

18. **`_service` suffix on function names** — Function names must be descriptive enough to stand alone. Appending `_service` (e.g. `process_point_service`, `fill_grammar_contexts_service`) is a naming crutch — it adds no meaning and signals the name itself isn't descriptive enough. Flag any function whose name ends in `_service` and suggest a rename that makes the operation self-evident.
...
20. **Search route URL does not follow path convention** — Search routes must use `/search/{column}` where `{column}` is the column or attribute being searched on (e.g. `/search/vector`, `/search/name`). When searching via a column on a related model, the path must be `/search/{related_model_resource}/{column}` (e.g. `/search/example/vector` — `example` is the related model resource, `vector` is the column). Flag any search route whose path does not end with the column name, or that omits the related model segment when searching across a relationship.
...
2. **Router prefix convention** — `crud.py` must define `router = APIRouter()` with no prefix. The resource prefix (e.g. `/playbooks`) must be applied in the package's `__init__.py` via `APIRouter(prefix="...")`. Other route files (`match.py`, `grammar.py`, etc.) follow the same convention — no prefix on the file's own router, prefix set in `__init__.py`. Flag any `crud.py` whose router carries a prefix directly.
...
15. **Intermediate `routes/` layer inside a package** — Route folders must sit directly under `(package-name)/` (e.g. `negotiations/clauses/crud.py`). An intermediate `routes/` directory (e.g. `negotiations/routes/clauses/crud.py`) violates the package layout. Flag any package that has a `routes/` subdirectory containing the route folders instead of the route folders sitting at the package root.

16. **Package-level `tools.py`** — A `tools.py` at the package root (e.g. `negotiations/tools.py`) is only valid if its helpers are genuinely shared across multiple route folders in that package. Flag any package-level `tools.py` whose functions are only used by a single route folder — those helpers should live in that folder's own `tools.py`.
...
13. **Raw hex codes in Tailwind** — No `text-[#abc123]` or `bg-[#...]` in className strings. Colors must come from `app/globals.css`. Flag all occurrences.
...
5. **Component placement not confirmed** — Generic reusable components belong in `frontend/components/`. Page-specific components belong in the page's `components/` subfolder. Flag components in the wrong location.
...
6. **`ondelete` not set on `ForeignKey` when ORM cascade is set** — When `cascade="all, delete-orphan"` is on the relationship, the `ForeignKey` must also have `ondelete="CASCADE"` so DB-level deletes (e.g. raw SQL, migrations) behave consistently. Flag the mismatch.
...
9. **FK column pointing to wrong table or column** — Check that every `ForeignKey("table.column")` string matches an existing table name (lowercase singular) and `id` column. Flag any that reference a table or column that does not exist.

10. **Missing `index=True` on FK column** — Foreign key columns used in joins or lookups should have `index=True`. Flag FK columns without an index.

11. **`default` used where `server_default` is needed** — A Python-side `default` is set by SQLAlchemy only when inserting via the ORM. Raw SQL inserts and migration-generated rows won't get the value. For database-generated defaults (timestamps, counters), use `server_default`. Flag `default=` on columns that should be DB-generated.

12. **Ambiguous relationship without `foreign_keys`** — When a model has more than one FK to the same related table, SQLAlchemy raises `AmbiguousForeignKeysError` unless `foreign_keys=[...]` is set on the relationship. Flag relationships that could be ambiguous.

13. **Model not imported in `alembic/env.py`** — For Alembic's `--autogenerate` to detect a model, the model file must be imported (directly or via a package `__init__`) before `target_metadata` is read. Flag any model file not reachable from the Alembic env import chain.

14. **Timestamp columns added without request** — `created_at` and `updated_at` must not be added unless explicitly asked. Flag any model that includes them without a recorded instruction.

15. **Column defined after a relationship** — SQLAlchemy allows it but it violates the project ordering convention and is confusing. Flag any data column defined below a `relationship(...)` in the same model.
...
3. **Relationship attribute not named by role** — The attribute name must reflect its role from this model's perspective (e.g. `clauses_authored`, `witnessing`). Flag generic names like `accounts`, `items`, or `related`.
...
5. **`__tablename__` casing** — Must be lowercase singular (`"account"`, not `"accounts"` or `"Account"`). Flag mismatches.
...
15. **`Note: ` comment rule applies to model files** — The same comment policy as the rest of the backend applies: no unapproved comments. Quote any comment that does not begin with `Note: ` so the user can decide.
...
3. **FK column not directly above its relationship** — The FK column (e.g. `account_id`) must be placed directly above the `relationship(...)` it backs with no blank line between them — they read as a pair. Flag any FK column separated from its relationship.
...
6. **Model file not named after its class** — The file name must match the model class name in snake_case (`Account` → `account.py`, `PlaybookClause` → `playbook_clause.py`). Flag mismatches.
...
9. **`hybrid_property` or `@property` added without request** — These are not part of the standard model pattern. Flag any property decorator on a model class that was not explicitly requested.
